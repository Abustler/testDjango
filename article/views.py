...
@csrf_exempt
def article_columm(request):
    if request.method == 'GET':
        columns = ArticleColumn.objects.filter(user=request.user)   # 用get时,如column不存在则会出错
...
def article_redit(request, article_id):
    if request.method=='GET':
        ...
        return render(request, 'article/column/article_redit.html', {'article':article,
